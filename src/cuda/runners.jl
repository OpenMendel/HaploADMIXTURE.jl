...
function em_p!(p_next, g::CuArray{UInt8, 2}, 
    q::CuArray{T, 2}, p::CuArray{T, 2}, I, J) where T
    fill!(p_next, zero(eltype(p_next)))
    kernel = @cuda launch=false em_p_kernel!(p_next, g, q, p, 1:I, 1:J)
    config = launch_configuration(kernel.fun)
    threads = config.threads
    threads_1d = Int(floor(sqrt(threads)))
    # println("threads: $threads")
    CUDA.@sync kernel(p_next, g, q, p, 1:I, 1:J; threads=(threads_1d, threads_1d), 
        blocks=(cld(I, threads_1d), cld(I, threads_1d)))
    p_next
end
...
function em_p!(d::CuAdmixData{T}, g_cu::CuMatrix{UInt8}) where T
    I, J, K = d.I, d.J, d.K
    em_p!(d.p_next, g_cu, d.q, d.p, I, J)
    d.p_next
end
